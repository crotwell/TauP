...
public class TauP_Table extends TauP_Time {
	...
	0.00f,   1.00f,   2.00f,   3.00f,   4.00f,   5.00f,  10.00f,  15.00f,  19.00f,  21.00f,
	25.00f,  30.00f,  33.00f,  35.00f,  40.00f,  45.00f,  49.00f,  51.00f,  55.00f,  60.00f,
	70.00f,  80.00f,  90.00f, 100.00f, 120.00f, 140.00f, 160.00f, 180.00f, 200.00f, 220.00f,
	240.00f, 260.00f, 280.00f, 300.00f, 350.00f, 400.00f, 450.00f, 500.00f, 550.00f, 600.00f };
	...
	8.00f,   8.10f,   8.20f,   8.30f,   8.40f,   8.50f,   8.60f,   8.70f,   8.80f,   8.90f,
	9.00f,   9.10f,   9.20f,   9.30f,   9.40f,   9.50f,   9.60f,   9.70f,   9.80f,   9.90f,
	...
	50.00f,  51.00f,  52.00f,  53.00f,  54.00f,  55.00f,  56.00f,  57.00f,  58.00f,  59.00f,
	60.00f,  61.00f,  62.00f,  63.00f,  64.00f,  65.00f,  66.00f,  67.00f,  68.00f,  69.00f,
	70.00f,  71.00f,  72.00f,  73.00f,  74.00f,  75.00f,  76.00f,  77.00f,  78.00f,  79.00f,
	80.00f,  81.00f,  82.00f,  83.00f,  84.00f,  85.00f,  86.00f,  87.00f,  88.00f,  89.00f,
	90.00f,  91.00f,  92.00f,  93.00f,  94.00f,  95.00f,  96.00f,  97.00f,  98.00f,  99.00f,
	100.00f, 105.00f, 110.00f, 115.00f, 120.00f, 125.00f, 130.00f, 135.00f, 140.00f, 145.00f,
	150.00f, 155.00f, 160.00f, 165.00f, 170.00f, 175.00f, 180.00f  };
	...
	if (headerFile != null) {
		...
		for (int i=0; i< distances.length; i++) {
			...
			if (head.ttype == StreamTokenizer.TT_WORD) {
			    Alert.error(
					"Expected a distance sample, but got ",
					head.sval);
			} else {
			    Alert.error(
					"Expected a distance sample, but got ",
    ...
    protected void locsatTable(DataOutputStream dos) 
	throws TauModelException,IOException {
	Arrival[] arrivals;
	...
	Format decimal7 = new Format("%-7d");
	double maxDiff =  
	    Double.valueOf(toolProps.getProperty("taup.table.locsat.maxdiff", 
						 "105.0")).doubleValue();
	
	dos.writeBytes("n # "+getPhaseNameString()+" travel-time tables for "+
		       modelName+" structure. (From TauP_Table)\n");
	dos.writeBytes(decimal7.form(depths.length)+"# number of depth samples\n");
	for (int depthNum = 0; depthNum < depths.length; depthNum++) {
	    dos.writeBytes(float7_2.form(depths[depthNum]));
	    if (depthNum % 10 == 9) { dos.writeBytes("\n"); }
	}
	if ((depths.length-1) % 10 != 9) { dos.writeBytes("\n"); }
	
	dos.writeBytes(decimal7.form(distances.length)+"# number of distances\n");
	for (int distNum = 0; distNum < distances.length; distNum++) {
	    dos.writeBytes(float7_2.form(distances[distNum]));
	    if (distNum % 10 == 9) { dos.writeBytes("\n"); }
	}
	if ((distances.length-1) % 10 != 9) { dos.writeBytes("\n"); }
