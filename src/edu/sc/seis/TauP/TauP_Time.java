...
public class TauP_Time {
    ...
    /** TauModel derived from tMod by correcting it
     *  for a non-surface source. */
    protected transient TauModel tModDepth;

    /** vector to hold the SeismicPhases for the phases
     *  named in phaseNames. */
     ...
     *  already loaded.
     *  @throws TauModelException if the file can't be found or is corrupted
     *  in some way.
     */
    ...
    if (phaseName.equalsIgnoreCase("ttp") ||
        ...
        appendPhaseName(new PhaseName(phaseName));
    }
    }
    ...
    return Double.valueOf(toolProps.getProperty("taup.source.depth", "0.0")).doubleValue();
    }
    ...
    public void loadTauModel(String modelName)
    ...
    /** Reads in list of phase names from a text file. So long as
     *  each phase name is separated by some whitespace, " " or newline
     *  or tab, it should read them fine. Also, comments are allowed, either
     *  # or // are comments to the end of the line while c style slash-star
     *  make a block a comment. */
    ...
    while (offset < phaseList.length()) {
        ...
        if (phaseSepIndex == phaseEntry.length()-2 &&
             ...
             * phase name. */
            appendPhaseName(new PhaseName(
                          phaseEntry.substring(0,phaseSepIndex),
                          Integer.valueOf(phaseEntry.substring(
                                               phaseSepIndex+1,phaseEntry.length())).intValue()));
        } else {
    ...
    while (offset < degList.length()) {
        ...
        degreesFound[numDegrees] = Double.valueOf(degEntry).doubleValue();
        offset = commaIndex+1;
        numDegrees++;
        } else {
    ...
    /* parses the standard command line args for the taup package. Other
     * tools that subclass this class will likely override this. */
        ...
        if (args[i].equalsIgnoreCase("-mod")
            ...
            *180.0/Math.PI;
            ...
            toolProps.put("taup.phase.list",
                      toolProps.getProperty("taup.phase.list","")+","+args[i+1]);
            } else {
            ...
            /* I don't know how to interpret this argument,
             * so pass it back */
            noComprendoArgs[numNoComprendoArgs++] = args[i];
            }
        } else {
    ...
    for (int j=0;j<arrivals.size();j++) {
        ...
        if (((Arrival)arrivals.elementAt(j)).puristName.length() >
        maxPuristNameLength) {
        maxPuristNameLength =
            ((Arrival)arrivals.elementAt(j)).puristName.length();
        }
    }
    ...
    if (!(onlyPrintRayP || onlyPrintTime)) {
        out.write("\nModel: "+modelName+"\n");
        out.write("Distance   Depth   "+
              phaseFormat.form("Phase")+
              "   Travel    Ray Param   Purist    Purist\n");
        out.write("  (deg)     (km)   "+
              phaseFormat.form("Name ")+
        ...
        out.write("  "+outForms.formatTime(currArrival.time)+
              "  "+outForms.formatRayParam(Math.PI/180.0*currArrival.rayParam)+"   ");
        ...
        currArrival = (Arrival)arrivals.elementAt(j);
        out.write(String.valueOf(
                     (float)(Math.PI/180.0*currArrival.rayParam))+" ");
        }
        out.write("\n");
    }
    out.write("\n");
    ...
    if (tMod == null
        || tMod.sMod.vMod.getModelName() !=
        toolProps.getProperty("taup.model.name",
                  "iasp91")) {
    ...
    if ((degrees != Double.MAX_VALUE ||
         (stationLat != Double.MAX_VALUE && stationLon != Double.MAX_VALUE &&
          eventLat != Double.MAX_VALUE && eventLon != Double.MAX_VALUE))) {
        /* enough info given on cmd line, so just do one calc. */
        if (degrees == Double.MAX_VALUE) {
        degrees = SphericalCoords.distance(stationLat, stationLon,
                           eventLat, eventLon);
        azimuth = SphericalCoords.azimuth(
                          eventLat, eventLon, stationLat, stationLon);
        backAzimuth = SphericalCoords.azimuth(
                              stationLat, stationLon, eventLat, eventLon);
        }
        depthCorrect(depth);
        calculate(degrees);
        printResult(dos);
        ...
        case 'd':
            ...
            if (tokenIn.ttype == tokenIn.TT_EOF ||
                            (tokenIn.ttype == tokenIn.TT_WORD &&
                             (tokenIn.sval.equalsIgnoreCase("q") ||
                              tokenIn.sval.equalsIgnoreCase("quit") ||
                              tokenIn.sval.equalsIgnoreCase("exit") ||
                              tokenIn.sval.equalsIgnoreCase("bye")))) {
                readMode = 'q';
        ...
        case 'a':
            ...
            Alert.warning("Expected a number.",
                                      "got "+tokenIn+" instead.");
            printHelp();
            break;
            }
            ...
            break;
        case 'b':
            //event to station back azimuth (ie station to event azimuth)
        ...
        case 'm':
            ...
            try {
                ...
                Alert.warning("Model file "+ modelName+
