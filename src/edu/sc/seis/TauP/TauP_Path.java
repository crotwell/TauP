...
public class TauP_Path extends TauP_Pierce {
	...
	for (int i=0;i<arrivals.size();i++) {
		...
		if (j < currArrival.path.length-1 && (currArrival.rayParam != 0.0 &&
						      180.0/Math.PI*(currArrival.path[j+1].dist-currArrival.path[j].dist)
						      > maxPathInc)) {
		    // interpolate to steps of at most maxPathInc degrees for path
	            maxInterpNum = (int)Math.ceil(
						  (currArrival.path[j+1].dist-currArrival.path[j].dist) *
						  180.0 / Math.PI / maxPathInc);
	            for (interpNum=1;interpNum < maxInterpNum; interpNum++) {
			calcTime += (currArrival.path[j+1].time-currArrival.path[j].time)/maxInterpNum;
			if (longWayRound) {
			    calcDist -= (currArrival.path[j+1].dist-
					 currArrival.path[j].dist)/maxInterpNum * 180.0/Math.PI;
			} else {
			    calcDist += (currArrival.path[j+1].dist-
					 currArrival.path[j].dist)/maxInterpNum * 180.0/Math.PI;
			}
			...
			calcDepth = prevDepth+interpNum *
			    (currArrival.path[j+1].depth-prevDepth)/maxInterpNum;
    ...
    protected void printLatLon(Writer out, double calcDist) 
	throws IOException 
    {
	double lat, lon;
	...
	if ( eventLat != Double.MAX_VALUE &&
	    ...
	    out.write("  " +outForms.formatLatLon(lat)+
		   ...
		   eventLon != Double.MAX_VALUE) {
	    azimuth = SphericalCoords.azimuth(eventLat, eventLon,
		...
		psFile = outFile+".ps";
            ...
            for (int j=0; j<2; j++) {
                ...
                for (int i=0;i<tMod.tauBranches[j].length;i++) {
                    dos.writeBytes("0.0 0.0 "+ 
                                   (float)(
                                           (tMod.getRadiusOfEarth()-tMod.tauBranches[j][i].getBotDepth()) *
                                           mapWidth / tMod.getRadiusOfEarth())+"\n");
                }
            }
