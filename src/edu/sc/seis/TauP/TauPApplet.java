...
public class TauPApplet extends Applet implements ActionListener, ItemListener {
	...
	try {
	    InputStream modelStream = getClass().getResourceAsStream("/StdModels/"+modelName+".taup");
	...
	try {
		...
		URL jarURL = new URL(
				     getCodeBase()+getParameter("ARCHIVE"));
		...
		while (jarEntry != null) {
			...
			getModel.addItem(jarEntry.getName().substring(
		...
		if (manifestStream != null) {
			...
			if (manifest.ttype == StreamTokenizer.TT_WORD &&
			    ...
			    getModel.addItem(manifest.sval.substring(0, manifest.sval.length()-5));
			}
		    }
	...
	if (target == "Calculate") { 
		...
		// update depth. Note this only recomputes the TauModel if the depth has changed
		depth = Double.valueOf(depthField.getText()).doubleValue();
		tool.depthCorrect(depth);

		// calculate the times for each phase at the given distance
		distance = Double.valueOf(distanceField.getText()).doubleValue();
		...
		if (textOrPlot.getLabel().equals("Plot")) {
		    ...
		    if (tool.getClass().getName().equals("edu.sc.seis.TauP.TauP_Time")) {
			...
			textArea.append(result.toString());
			textArea.append("Done!\n");
		    } else if (tool.getClass().getName().equals("edu.sc.seis.TauP.TauP_Pierce")) {
			...
			for (int i=0; i< disconDepths.length; i++) {
			    disconRadius[i] = tool.getTauModel().getRadiusOfEarth()-disconDepths[i];
			}
			...
			plotArea.repaint();
		    } else if (tool.getClass().getName().equals("edu.sc.seis.TauP.TauP_Path")) {
			double[] disconDepths = tool.getDisconDepths();
			double[] disconRadius = new double[disconDepths.length];
			for (int i=0; i< disconDepths.length; i++) {
			    disconRadius[i] = tool.getTauModel().getRadiusOfEarth()-disconDepths[i];
			...
			plotArea.repaint();
		    } else if (tool.getClass().getName().equals("edu.sc.seis.TauP.TauP_Curve")) {
