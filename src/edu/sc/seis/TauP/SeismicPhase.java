...
public class SeismicPhase implements Serializable, Cloneable {
    ...
    /** Used by addToBranch when the path turns within 
     *  a segment. We assume that no ray will turn downward so turning
     *  implies turning from downward to upward, ie U. */
    protected static final int TURN = 0;
   
    /** Used by addToBranch when the path reflects off the top 
     *  of the end of a segment, ie ^. */
    protected static final int REFLECTTOP = 1;

    /** Used by addToBranch when the path reflects off the bottom 
     *  of the end of a segment, ie v. */
    ...
    /** @param phaseName String containing a name of the phase.
     *  @param tMod Tau model to be used to construct the phase. */
	...
	double tempDeg = deg;
	if (tempDeg<0.0) {tempDeg *= -1.0;}    // make sure deg is positive
	while (tempDeg>360.0) {tempDeg -= 360.0;} // make sure it is less than 360
	if (tempDeg>180.0) {tempDeg = 360.0-tempDeg;} // make sure less than or equal to 180
	 ...
	 * distance, for instance PKKKKP might wrap all of the way 
	 * around. A special case exists at 180, so we skip the second case
	 * if tempDeg==180. */
	int n=0;
	Arrival newArrival;
	double searchDist;
	while (n*2.0*Math.PI+radDist <= maxDistance) {
            /* Look for arrivals that are radDist + 2nPi, ie rays that
             * have done more than n laps. */
		...
		if (searchDist == dist[rayNum+1] && 
			...
			(dist[rayNum]- dist[rayNum+1])+ rayParams[rayNum+1];
            ...
            /* Look for arrivals that are 2(n+1)Pi-radDist, ie rays that
             * have done more than one half lap plus some number of
             * whole laps. */
	    searchDist = (n+1)*2.0*Math.PI - radDist;
 
	    if (tempDeg != 180) {
		for (int rayNum=0; rayNum < (dist.length-1); rayNum++) {
		    if (searchDist == dist[rayNum+1] && rayNum+1 != dist.length-1) {
			/* So we don't get 2 arrivals for the same ray. */
			continue;
	            } else if ((dist[rayNum] - searchDist)*
			       (searchDist - dist[rayNum+1]) >= 0.0) {
			...
			if (DEBUG) {
			    System.err.println(
					       "SeismicPhase "+name+", found arrival:\n"+
					       "dist "+(float)(180/Math.PI*dist[rayNum])+
					       " "+(float)(180/Math.PI*searchDist)+
					       " "+(float)(180/Math.PI*dist[rayNum+1]));
			}
			double arrivalTime = (searchDist-dist[rayNum]) /
			...
			double arrivalRayParam = (searchDist-dist[rayNum])*
			    ...
			    (dist[rayNum+1]- dist[rayNum])+ rayParams[rayNum];
     ...
     *  because there are SKS ray parameters that cannot propagate 
     *  from the cmb into the mantle as a p wave.  */
    protected void phaseConversion(TauModel tMod, int fromBranch, int endAction,
	...
	if (endAction == TURN) {
	    ...
	    maxRayParam = Math.min(maxRayParam,
				   tMod.getTauBranch(fromBranch, isPtoS).getMinTurnRayParam());
	    maxRayParam = Math.min(maxRayParam,
				   tMod.getTauBranch(fromBranch, ! isPtoS).getMinTurnRayParam());
	    ...
	    maxRayParam = Math.min(maxRayParam,
				   tMod.getTauBranch(fromBranch, isPtoS).getMaxRayParam());
	    maxRayParam = Math.min(maxRayParam,
				   tMod.getTauBranch(fromBranch-1, ! isPtoS).getMinTurnRayParam());
	} else if (endAction == TRANSDOWN) {
	    maxRayParam = Math.min(maxRayParam,
				   tMod.getTauBranch(fromBranch, isPtoS).getMinRayParam());
	    maxRayParam = Math.min(maxRayParam,
     ...
     * branchSeq, in order. Also, currBranch is set correctly based on 
     * the value of endAction. endAction can be one of TRANSUP, TRANSDOWN,
     * REFLECTTOP, REFLECTBOT, or TURN. */
    protected void addToBranch(TauModel tMod, int startBranch, int endBranch,
			       boolean isPWave, int endAction) throws TauModelException {
	...
	if (endAction == TURN) {
	    ...
	    minRayParam = Math.max(minRayParam,
				   tMod.getTauBranch(endBranch, isPWave).getMinTurnRayParam());
	} else if (endAction == REFLECTTOP) {
	    endOffset = 0;
	    isDownGoing = false;
	    maxRayParam = Math.min(maxRayParam,
	    ...
	    isDownGoing = true;
	    maxRayParam = Math.min(maxRayParam,
				   tMod.getTauBranch(endBranch, isPWave).getMinRayParam());
	} else {
	    throw new TauModelException("Illegal endAction: endAction="+endAction);
	}
	if (isDownGoing) {
		...
		for (int i=startBranch; i <= endBranch; i++) {
		    System.out.println("i="+i+" isDownGoing="+isDownGoing+
				       " isPWave="+isPWave+" startBranch="+startBranch+" endBranch="
				       +endBranch+" "+endAction);
		}
	    }
	} else {
	...
	for (int i=0;i<tMod.getNumBranches();i++) {
	    ...
	    if (Math.abs(disconDepth - tBranch.topDepth) < disconMax && 
		! tMod.isNoDisconDepth(tBranch.topDepth)) {
		disconBranch = i;
		disconMax =
		    Math.abs(disconDepth - tBranch.topDepth);
	    }
	}
    ...
    /** Constructs a branch sequence from the given phase name and
     *  tau model. */
	...
	if (currLeg.startsWith("P") || currLeg.startsWith("S")) {
	    ...
	    throw new TauModelException("First phase not recognized: "+currLeg+
					" must be one of P, Pg, Pn, Pdiff, p or the S equivalents");
	}
      
	/* Set maxRayParam to be a horizontal ray leaving the source and
	 * set minRayParam to be a vertical (p=0) ray. */
	if (tMod.getSourceBranch() != 0) {
	    maxRayParam 
		= Math.max( tMod.getTauBranch(tMod.getSourceBranch()-1,
	...
	for (int legNum=0;legNum<legs.size()-1;legNum++) {
		 ...
		 * so do nothing. This makes sure we us the correct maxRayParam
		 * from the correct TauBranch within the outer core. In other
		 * words K has a high slowness zone if it entered the outer
		 * core as a mantle P wave, but doesn't if it entered as a
		 * mantle S wave. It shouldn't matter for inner core to outer
		 * core type legs. */
	    }
		...
		if (nextLeg.startsWith("v")) {
		    throw new TauModelException(
						"p and s must always be up going "+
			...
			throw new TauModelException("Phase not recognized: "+currLeg+
						    " followed by "+nextLeg+" when currBranch="+
						    currBranch+" > disconBranch="+disconBranch);
		    }
		} else if (nextLeg.equals("m") &&
			   currBranch >= tMod.getMohoBranch()) {
		    addToBranch(tMod, currBranch, tMod.getMohoBranch(),
				isPWave, TRANSUP);
		    ...
		    addToBranch(tMod, currBranch, disconBranch,
				isPWave, TRANSUP);
		} else {
		    throw new TauModelException("Phase not recognized: "+currLeg+
						" followed by "+nextLeg);
		...
		if (nextLeg.equals("P") ||
			...
			addToBranch(tMod, currBranch, tMod.getCmbBranch()-1, isPWave,
				    TURN);
		    }
		    addToBranch(tMod, currBranch, 0, isPWave, REFLECTTOP);
		} else if (nextLeg.startsWith("v")) {
		    disconBranch = closestBranchToDepth(tMod, nextLeg.substring(1));
		    if (currBranch <= disconBranch-1) {
			addToBranch(tMod, currBranch, disconBranch-1, isPWave,
				    REFLECTBOT);
		    } else {
			throw new TauModelException("Phase not recognized: "+currLeg+
						    " followed by "+nextLeg+" when currBranch="+
			...
			addToBranch(tMod, currBranch, disconBranch,
				    isPWave, REFLECTTOP);
		    } else if (prevLeg.startsWith("^") ||
			       prevLeg.equals("P") ||
			       prevLeg.equals("S") ||
			       prevLeg.equals("p") ||
			       prevLeg.equals("s") ||
			       prevLeg.equals("START")) {
			addToBranch(tMod, currBranch, tMod.getCmbBranch()-1,
				    isPWave, TURN);
			addToBranch(tMod, currBranch, disconBranch,
				    isPWave, REFLECTTOP);
		    } else if ((prevLeg.startsWith("v") && disconBranch <
				closestBranchToDepth(tMod, prevLeg.substring(1))) ||
			       (prevLeg.equals("m") &&
			...
			// downgoing section, must look at the leg after the next
			// leg to determine whether to convert on the downgoing or
			...
			if (nextNextLeg.equals("p") || nextNextLeg.equals("s")) {
			    ...
			    addToBranch(tMod, currBranch, disconBranch,
					isPWave, TRANSUP);
			} else if (nextNextLeg.equals("P") || nextNextLeg.equals("S")) {
			    if (disconBranch > currBranch) {
				// discon is below current loc
		               	addToBranch(tMod, currBranch, disconBranch-1,
					    isPWave, TRANSDOWN);
			    } else {
			    ...
			    throw new TauModelException("Phase not recognized: "+
							currLeg+" followed by "+nextLeg+" followed by "+
							nextNextLeg);
			}
		    }
		} else {
		...
		if (currLeg.equals("Pdiff") || currLeg.equals("Sdiff")) {
		     ...
		     * thinking we are turning, but then make the maxRayParam
		     * equal to minRayParam, which is the deepest turning ray. */
		    if (maxRayParam >= tMod.getTauBranch(
							 tMod.getCmbBranch()-1, isPWave).getMinTurnRayParam() &&
			minRayParam <= tMod.getTauBranch(
							 tMod.getCmbBranch()-1, isPWave).getMinTurnRayParam()) {
			...
			maxRayParam = -1;
			if (DEBUG) {
			    System.out.println("Cannot have the head wave "+currLeg+
					       " within phase "+name+
			...
			/* Pg, Pn, Sg and Sn must be above the moho and so is not 
			 * valid for rays coming upwards from below, possibly due
			 * to the source depth. Setting maxRayParam = -1
			 * effectively disallows this phase. */
			maxRayParam = -1;
			if (DEBUG) {
			    System.out.println("(currBranch >= tMod.getMohoBranch() "+
					       currBranch+" "+tMod.getMohoBranch()+
					       " so there cannot be a "+currLeg+
					       " phase for this sourceDepth and/or path.");
			}
			return;
		    }
		    if (currLeg.equals("Pg") || currLeg.equals("Sg")) {
			addToBranch(tMod, currBranch, tMod.getMohoBranch()-1,
				    isPWave, TURN);
			addToBranch(tMod, currBranch, 0, isPWave, REFLECTTOP);
			 ...
			 * make the minRayParam equal to maxRayParam, which 
			 * is the head wave ray. */
			if (maxRayParam >= tMod.getTauBranch(
							     tMod.getMohoBranch(), isPWave).getMaxRayParam() &&
			    minRayParam <= tMod.getTauBranch(
							     tMod.getMohoBranch(), isPWave).getMaxRayParam()) {
		...
		if (nextLeg.equals("P") || nextLeg.equals("S")) {
			...
			prevLeg.equals("S") ||
			prevLeg.equals("K")) {
			addToBranch(tMod, currBranch, tMod.getIocbBranch()-1,
				    isPWave, TURN);
		    }
		    addToBranch(tMod, currBranch, tMod.getCmbBranch(), isPWave,
				TRANSUP);
		} else if (nextLeg.equals("K")) {
		    if (prevLeg.equals("P") ||
			prevLeg.equals("S") ||
		    ...
		    addToBranch(tMod, currBranch, tMod.getIocbBranch()-1, isPWave,
				TRANSDOWN);
		...
		addToBranch(tMod, currBranch, tMod.getNumBranches()-1, isPWave,
			    TURN);
		if (nextLeg.equals("I") || nextLeg.equals("J")) {
		    addToBranch(tMod, currBranch, tMod.getIocbBranch(), isPWave,
				REFLECTTOP);
		} else if (nextLeg.equals("K")) {
		    addToBranch(tMod, currBranch, tMod.getIocbBranch(), isPWave,
				TRANSUP);
    ...
    /** Tokenizes a phase name into legs, ie PcS becomes 'P'+'c'+'S' while
     *  p^410P would become 'p'+'^410'+'P'. Once a phase name has been 
     *  broken into tokens we can begin to construct the sequence of 
     *  branches to which it corresponds. Only minor error checking is 
     *  done at this point, for instance PIP generates an exception 
     *  but ^410 doesn't. It also appends "END" as the last leg.
     *  @throws TauModelException if the phase name cannot be tokenized. */
	...
	if (name.endsWith("kmps")) {
	    try {
		double vel = 
		    Double.valueOf(name.substring(0,name.length()-4)).doubleValue();
		...
		if (offset+1 == name.length() ||
		    ...
		    offset = offset+2;
							 ...
							 name.substring(offset, offset+5).equals("Sdiff") ||
							 name.substring(offset, offset+5).equals("Pdiff"))) {
		...
		if ( name.charAt(offset+1) == 'm' ||
		    ...
		    while (offset+numDigits+1 < name.length() &&
			   Character.isDigit(name.charAt(offset+numDigits+1))) {
			numDigits++;
		    }
		    legs.addElement(name.substring(offset, offset+numDigits+1));
	    ...
	    } else if (Character.isDigit(name.charAt(offset)) || name.charAt(offset)== '.') {
	...
	for (int legNum=0;legNum<legs.size()-1;legNum++) {
		...
		disconBranch = closestBranchToDepth(tMod, 
						    currLeg.substring(1));
		legDepth = tMod.getTauBranch(disconBranch,true).topDepth;
     ...
     *  @throws TauModelException if the topDepth of the high slowness zone
     *		is not contained within the TauModel. This should never happen
     *    and would indicate an invalid TauModel. */
	...
	if (name.endsWith("kmps")) {
		...
		Double.valueOf(name.substring(0,name.length()-4)).doubleValue();
	    dist[1] = 2*Math.PI;
	    time[1] = 2*Math.PI*tMod.radiusOfEarth/
		Double.valueOf(name.substring(0,name.length()-4)).doubleValue();
	...
	if (maxRayParamIndex == 0 && 
	    ...
	    System.arraycopy(tMod.rayParams, 0, rayParams, 0,
	    ...
	    System.arraycopy(tMod.rayParams, maxRayParamIndex, rayParams, 0,
	...
	for (int j=0; j<tMod.getNumBranches(); j++) {
		...
		for (int i=maxRayParamIndex; i<minRayParamIndex+1; i++) {
			...
			tMod.getTauBranch(j, PWAVE).dist[i];
		...
		for (int i=maxRayParamIndex; i<minRayParamIndex+1; i++) {
			...
			tMod.getTauBranch(j, SWAVE).dist[i];
	...
	if (name.indexOf("Sdiff") != -1 || name.indexOf("Pdiff") != -1) {
		...
		time[1] = time[0] + maxDiffraction*Math.PI/180.0*minRayParam;
	    }
	...
	for (int j=0; j<dist.length; j++) {
	    if (dist[j] < minDistance) { minDistance = dist[j];}
	    if (dist[j] > maxDistance) { maxDistance = dist[j];}
	}
	...
	for (dummy=0 , isPWave=true; dummy < 2; dummy++, isPWave=false) {
		...
		if (maxRayParam > hsz[i].rayParam && hsz[i].rayParam > minRayParam) {
		    /* There is a high slowness zone within our ray parameter 
		     * range so we might need to add a shadow zone.
		     * We need to check to see if this wave type, P or S, is
		     * part of the phase at this depth/ray parameter.
		     */
			...
			// check for downgoing legs that cross the high slowness zone
			...
			for (int j=0; j<tMod.getNumBranches(); j++) {
			    if (timesBranches[0][j] != 0 &&
				tMod.getTauBranch(j, PWAVE).getTopDepth() < hsz[i].topDepth) {
				newdist[hSZIndex] += timesBranches[0][j] * 
				    tMod.getTauBranch(j, PWAVE).dist[maxRayParamIndex+hSZIndex-indexOffset];
				newtime[hSZIndex] += timesBranches[0][j] * 
				    tMod.getTauBranch(j, PWAVE).time[maxRayParamIndex+hSZIndex-indexOffset];
			    }
			    if (timesBranches[1][j] != 0 &&
				tMod.getTauBranch(j, SWAVE).getTopDepth() < hsz[i].topDepth) {
				newdist[hSZIndex] += timesBranches[1][j] * 
				    tMod.getTauBranch(j, SWAVE).dist[maxRayParamIndex+hSZIndex-indexOffset];
				newtime[hSZIndex] += timesBranches[1][j] * 
				    tMod.getTauBranch(j, SWAVE).time[maxRayParamIndex+hSZIndex-indexOffset];
			    }
			}
	
			System.arraycopy(dist,hSZIndex,newdist,hSZIndex+1,
					 dist.length-hSZIndex);
			System.arraycopy(time,hSZIndex,newtime,hSZIndex+1,
					 time.length-hSZIndex);
			System.arraycopy(rayParams,hSZIndex,newrayParams,hSZIndex+1,
	...
	for(int arrivalNum=0; arrivalNum<arrivals.size(); arrivalNum++) {
	    ...
	    currArrival.pierce[0] = new TimeDist(distRayParam, 0.0, 0.0,
						 tMod.sourceDepth);
	    numAdded = 1;
         
            /* Loop from 0 but already done 0, so the pierce
             * point when the ray leaves branch i is stored in i+1.
		...
		if (DEBUG) {
		    System.out.println(i+" branchNum ="+branchNum+
				       " downGoing="+
				       (Boolean)downGoing.elementAt(i)+
				       "  isPWave="+ isPWave);
		}

		/* Save the turning depths for the ray parameter for both P
		 * and S waves. This way we get the depth correct for any 
		 * rays that turn within a layer. We have to do this on a
		 * per branch basis because of converted phases, e.g. SKS. */
		try {
		    if (distRayParam > 
			tMod.getTauBranch(branchNum, 
					  isPWave).getMaxRayParam()) {
			turnDepth = tMod.getTauBranch(branchNum, 
						      isPWave).getTopDepth();
		    } else if (distRayParam <= 
			       tMod.getTauBranch(branchNum, 
						 isPWave).getMinRayParam()) {
			turnDepth = tMod.getTauBranch(branchNum, 
						      isPWave).getBotDepth();
		    } else {
			if (isPWave || 
			    tMod.sMod.depthInFluid((tMod.getTauBranch(branchNum, isPWave).getTopDepth() +
						    tMod.getTauBranch(branchNum, isPWave).getBotDepth())/2.0)) {
			    turnDepth = tMod.sMod.findDepth(distRayParam,
							    tMod.getTauBranch(branchNum, isPWave).getTopDepth(), 
							    tMod.getTauBranch(branchNum, isPWave).getBotDepth(),
							    PWAVE);
			} else {
			    turnDepth = tMod.sMod.findDepth(distRayParam,
							    tMod.getTauBranch(branchNum, isPWave).getTopDepth(), 
		...
		if (name.indexOf("Pdiff") != -1 || name.indexOf("Pn") != -1 ||
		    ...
		    distA = tMod.getTauBranch(branchNum, isPWave).dist[rayNum];
		    timeA = tMod.getTauBranch(branchNum, isPWave).time[rayNum];
		    distB = tMod.getTauBranch(branchNum, isPWave).dist[rayNum];
		    timeB = tMod.getTauBranch(branchNum, isPWave).time[rayNum];
		} else {
		    distA = tMod.getTauBranch(branchNum, isPWave).dist[rayNum];
		    timeA = tMod.getTauBranch(branchNum, isPWave).time[rayNum];
		    distB = tMod.getTauBranch(branchNum, isPWave).dist[rayNum+1];
		...
		if (((Boolean)downGoing.elementAt(i)).booleanValue()) {
		    branchDepth = 
			Math.min(tMod.getTauBranch(branchNum, 
						   isPWave).getBotDepth(), 
				 turnDepth);
		} else {
		    branchDepth = 
		...
		if (Math.abs(prevBranchTime - branchTime) > 1e-10) {
		    currArrival.pierce[numAdded++] = 
			new TimeDist(distRayParam, 
				     branchTime, 
				     branchDist, 
				     branchDepth);
		    if (DEBUG) {
             ...
             * waves. It is assumed that a phase can be a diffracted wave or
	     * a head wave, but not both. Nor can it be a head wave or 
	     * diffracted wave for both P and S. */
	    if (name.indexOf("Pdiff") != -1 || name.indexOf("Pn") != -1 ||
		name.indexOf("Sdiff") != -1 || name.indexOf("Sn") != -1) {
		...
		if (name.indexOf("Pdiff") != -1 || name.indexOf("Sdiff") != -1) {
			...
			diffTD[i+j+1] = new TimeDist(diffTD[i].p,
						     diffTD[i+j].time+refractDist/numFound *diffTD[i].p,
						     diffTD[i+j].dist+refractDist/numFound, tMod.mohoDepth);
			i++;
			j++;
		    }

		    System.arraycopy(currArrival.pierce,i,diffTD,i+j,numAdded-i);
	...
	for(int arrivalNum=0; arrivalNum<arrivals.size(); arrivalNum++) {
	    ...
	    tempTimeDist[0] = new TimeDist(currArrival.rayParam, 0.0, 0.0,
					   tMod.sourceDepth);
		...
		for (int i=0; i < branchSeq.size(); i++) {
		    ...
		    tempTimeDist = tMod.getTauBranch(branchNum, isPWave).path(
									      currArrival.rayParam, 
									      ((Boolean)downGoing.elementAt(i)).booleanValue(), tMod.sMod);
			...
			((Integer)branchSeq.elementAt(i+1)).intValue()
			== tMod.cmbBranch-1 && (
						name.indexOf("Pdiff") != -1 || name.indexOf("Sdiff") != -1)) {

			TimeDist[] diffTD = new TimeDist[1];
			diffTD[0] = new TimeDist(currArrival.rayParam,
						 (currArrival.dist - dist[0])*currArrival.rayParam,
						 currArrival.dist - dist[0], tMod.cmbDepth);
			...
			headTD[0] = new TimeDist(currArrival.rayParam,
						 (currArrival.dist - dist[0])/numFound *currArrival.rayParam,
						 (currArrival.dist - dist[0])/numFound, tMod.mohoDepth);
		...
		TimeDist cummulative = 
		    new TimeDist(currArrival.rayParam, 0.0, 0.0, currArrival.sourceDepth);
		TimeDist[] branchPath;
		int numAdded = 0;
		for (int i=0; i < pathList.size(); i++) {
	...
	if (legs.size() == 2 &&
	    (currToken.equals("Pdiff") || currToken.equals("Sdiff") ||
	     currToken.endsWith("kmps") ) &&
	    ((String)legs.elementAt(1)).equals("END")) {
	    return true;
	}

	/* Check first leg. */
	if (!(currToken.equals("Pg") ||
	      currToken.equals("Pb") ||
	      currToken.equals("Pn") ||
	      currToken.equals("Pdiff") ||
	      currToken.equals("Sg") ||
	      currToken.equals("Sb") ||
	      currToken.equals("Sn") ||
	      currToken.equals("Sdiff") ||
	      currToken.equals("P") ||
	      currToken.equals("S") ||
	      currToken.equals("p") ||
	      currToken.equals("s"))) {
	...
	for (int i=1; i<legs.size();i++) {
	    ...
	    if ((currToken.startsWith("P") ||
		 currToken.startsWith("S") ||
		 currToken.startsWith("p") ||
		 currToken.startsWith("s") ||
		 currToken.equals("m") ||
		 currToken.equals("c")
		 ) && (
	    ...
	    if ((prevToken.startsWith("P") ||
		 prevToken.startsWith("S") ||
		 prevToken.startsWith("p") ||
		 prevToken.startsWith("s") ||
		 prevToken.equals("m") ||
		 prevToken.equals("c")
		 ) && (
	...
	try {
	    ...
	    for (int i=0;i<branchSeq.size();i++) {
		newObject.branchSeq.addElement(new Integer(
							   ((Integer)branchSeq.elementAt(i)).intValue()));
   
	    }
	    for (int i=0;i<legs.size();i++) {
		newObject.legs.addElement(new Integer(
						      ((Integer)legs.elementAt(i)).intValue()));
	    }
	    newObject.dist = (double[])dist.clone();
	    newObject.time = (double[])time.clone();
	    newObject.rayParams = (double[])rayParams.clone();
	    for (int i=0;i<arrivals.size();i++) {
		newObject.arrivals.addElement(
					      (Arrival)((Arrival)arrivals.elementAt(i)).clone());
	    }
