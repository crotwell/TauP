...
public class TauP_Pierce extends TauP_Time {
   ...
   public TauP_Pierce(TauModel tMod) throws TauModelException {
	...
	double calcDepth, calcTime, calcDist;
	...
	for (int i=0;i<arrivals.size();i++) {
	    ...
	    out.write("> "+currArrival.name+ " at " + 
		      outForms.formatTime(currArrival.time)+" seconds at "+
		...
		if (!(onlyTurnPoints || onlyRevPoints || onlyUnderPoints || 
		      onlyAddPoints ) 
		    || (( onlyAddPoints && 
			  isAddDepth(currArrival.pierce[j].depth)) ||
			( onlyRevPoints && 
