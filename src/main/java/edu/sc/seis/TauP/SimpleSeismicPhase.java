...
public abstract class SimpleSeismicPhase implements SeismicPhase {
    ...
    @Override
    public List<ArrivalPathSegment> calcSegmentPaths(Arrival arrival) throws SlownessModelException, TauModelException, NoArrivalException {
        return calcSegmentPaths(arrival, new TimeDist(arrival.getRayParam(), 0, 0, arrival.getSourceDepth()), 0);
    }
     ...
     * @return
     */
    public List<ArrivalPathSegment> calcSegmentPaths(Arrival currArrival, TimeDist prevEnd, int prevIdx) throws SlownessModelException, TauModelException {
        int idx = prevIdx+1;
